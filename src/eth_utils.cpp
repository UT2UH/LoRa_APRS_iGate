...
namespace ETH_Utils {
        ...
        if (!Config.digi.ecoMode) {
            if (backUpDigiMode) {
                ...
                if (!EthConnected && EthCheck >= 15 * 60 * 1000) {
                    ...
                    Utils::println("*** LAN Reconnect Success (Stopping Backup Digi Mode) ***");
                    backUpDigiMode = false;
                    EthCounter = 0;
                }
            }
            ...
            if (!backUpDigiMode && !EthConnected && ((millis() - previousEthMillis) >= 30 * 1000)) {
                Utils::print((String)millis());
                Utils::println("Waiting for LAN Reconnect...");
                ...
                if (EthCounter >= 2) {
                    Utils::println("*** LAN lost. Starting BackUp Digi Mode ***");
                    backUpDigiMode = true;
    ...
    void startETH() {
        u_int8_t counter = 0;
        Network.onEvent(ETH_Utils::EthEvent);
        displayShow("", "Connecting to LAN:", "", "       ...", 0);
        Utils::print("Connecting to LAN: ");
        ETH.begin();
        ...
        if (EthConnected) {
            Utils::print("\nConnected as ");
            Utils::print(String(ETH.localIP()));
            Utils::print(" / MAC: ");
            Utils::println(ETH.macAddress());
    ...
    void setup() {
        if (Config.ethernet.use_lan && !Config.digi.ecoMode) 
            startETH();
    }
}
#endif